...
select bno, title, re_ref, re_seq, re_lev from board where bno=1540;



-- 첫번째 댓글
insert into board(bno,name,password,title,content,attach,re_ref,re_lev,re_seq)
values(board_seq.nextval,'댓글러','1234','re :댓글1','댓글작성',null,1540,1,1);

-- 원본글과 뎃글 그룹으로 가져오기
select bno, title, re_ref, re_seq, re_lev from board where re_ref = 1540 order by re_seq asc;

-- 댓글 작성 시 댓글을 최신 순으로 추출 할 수 있어야 함(re_seq사용)

-- 1.  기존 댓글의 re_seq 값을 업데이트
--      update board set re_seq = re_seq + 1 where re_ref = 원본글의re_ref and re_seq > 원본글의 re_seq
update board set re_seq = re_seq+1 where re_ref=1540 and re_seq > 0;
...
insert into board(bno,name,password,title,content,attach,re_ref,re_lev,re_seq)
values(board_seq.nextval,'댓글러','1234','re :댓글2','댓글작성2',null,1540,1,1);

-- 댓글의 댓글 작업
-- 업데이트
--  update board set re_seq = re_seq + 1 where re_ref = 원본글의re_ref and re_seq > 원본글의 re_seq

update board set re_seq = re_seq + 1 where re_ref = 1540 and re_seq > 1

--댓글의 댓글 삽입
insert into board(bno,name,password,title,content,attach,re_ref,re_lev,re_seq)
values(board_seq.nextval,'댓글러3','1234','rere :댓글3','댓글작성3',null,1540,2,2);
...
-- 페이지 나누기
-- MySQL : limit
-- Oracle : rownum(가상 칼럼) - 조회된 결과에 번호를 순서적으로 매겨줌
...
--결과 나옴 (뭐보다 작은 이런식으로 써야함)

select rownum, bno, title from board where rownum <= 10order by bno desc;
--rownum을 사용할 때 order by 절 사용시 order by 에 오는 컬럼은 index 로 설정되어 있어야 한다
...
from(select rownum as rnum, bno, title, name, regdate, readcount, re_ref, re_lev, re_seq
	from (select bno, title, name, regdate, readcount, re_refm re_lev, re_seq
		from board 
...
select bno, title, name, regdate, readcount, re_refm re_lev, re_seq --3번
from board -- 실행순서 1분
where bno >0 -- 2번
order by re_ref, re_seq asc; --4번

-- 2) 1) 결과에 rownum 부여후 10이하인 것만 추출
select rownum as rnum, bno, title, name, regdate, readcount, re_refm re_lev, re_seq
from (1번결과)
where rownum <=10

-- 3) 2)번 결과에서
select bno, title, name, regdate, readcount, re_refm re_lev, re_seq
from (2번결과)
where rnum > 0;


-- 1page : rownum <=10(start), rnum >0(end), 2page : rownmum <=20, rnum>10

-- 1 page
-- start = 1*10, end = (1-1) *!0

-- 2 page : 페이지 번호 * 한페이지당 보여줄 게시물 수 /(페이지번호 -1) * 한 페이지 당 보여줄 게시물 수
-- start = 2*10m end = (2-1) *!0
